...
pygame.display.set_icon(pygame.image.load('platformer.png'))
...
class Game:
	...
	def setScreen(self):
		width = int(player.level.width*Level.tilesize)
		height = int(player.level.height*Level.tilesize)
	...
	def draw(self):
		...
		pygame.display.update(player.rect.inflate(player.width*2.5, player.height*2.5))
...
class Level:
	tilesize = 48;
	numlevels = 0
	def __init__(self, layout, bg=(100, 160, 255), gravity=1):
		...
		self.tilesurface = pygame.Surface((round(self.width*Level.tilesize), round(self.height*Level.tilesize)))
		self.bg = bg
		...
		self.speeddivisor = 12  # The higher the number, the slower you fall
...
class Player:
	...
	def fall(self):
		if (not self.jumping) and (not self.incurrent) and (not self.hurtonspike) and (not self.hanging):
			...
			if (self.swimming and not self.checkIn('name', 'air')) and (gameInput.left or gameInput.right):
				fallspeed *= 0.5
			if not self.onground:# or self.swimming:
				if (not self.falling):
					self.momentum = fallspeed
				self.falling = True
				fall = fallspeed-self.momentum
				self.y += fall
				self.momentum *= 0.975
				print(self.momentum)
			else:
	...
	def control(self):
		if not self.hurtonspike:
			...
			if not self.moved and self.snaptogrid and not self.incurrent and not self.onconveyor:
				# snap to block if on very edge of another
				if abs(self.x-round(self.x)) < 0.25:
					self.snapX()
						
		if gameInput.up and self.canjump and (gameInput.KEYTAPPED or self.swimming) and (not self.jumping):
			self.jumping = True
			self.jump = 0
		if self.jumping:
		...
		if self.swimming:
			...
			if gameInput.up and self.canjump:
				self.y -= speed
			if gameInput.down and not self.checkOn('solid'):
				self.y += speed
	...
	def collision(self):
		...
		if self.checkAbove('name', 'monkey bars') and not gameInput.down:
			hanging = True
		else:
			hanging = False
		...
		if self.checkAround('name', 'goo block') or checkBlock(self.x+0.55, self.y+1, 'name') == 'goo block' or checkBlock(self.x-0.55, self.y+1, 'name') == 'goo block':
			...
			if gameInput.up and (gameInput.KEYTAPPED or self.checkAbove('name', 'goo block')):
				self.y -= speed
				#if (gameInput.left and self.cangoleft) or (gameInput.right and self.cangoright):
				gameInput.canjump = True
				self.jumping = True
			if gameInput.down and not self.checkOn():
				self.y += speed
		else:
			hanging = False
		...
		touchingspike = ((self.checkSide(-0.55, 'name', 'spike block') and gameInput.left) or (self.checkSide(0.55, 'name', 'spike block') and gameInput.right)) or self.checkOn('name', 'spike block') or (self.checkAbove('name', 'spike block') and (gameInput.up or self.jumping))
